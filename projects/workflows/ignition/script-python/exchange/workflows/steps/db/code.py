...
log = system.util.getLogger("exchange." + system.util.getProjectName().lower() +".steps.db")

@step(name="db.query")
def query(db_name, sql, args=None):
    """
    Run a prepared query against the specified Ignition DB connection.
            Returns list-of-dicts.

    Args:
        db_name (str): Ignition database connection name.
...
@step(name="db.update")
def update(db_name, sql, args=None):
    """
    ...
    """
    args = args or []
    n = system.db.runPrepUpdate(sql, args, db_name)
