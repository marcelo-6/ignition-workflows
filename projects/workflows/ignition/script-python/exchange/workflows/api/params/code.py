...
from exchange.workflows.engine.db import DB, now_ms
from exchange.workflows.api import service as workflowsService


log = system.util.getLogger("exchange." + system.util.getProjectName().lower() + ".api.params")
...
def expandTemplateForUi(templateObj, dbName=DEFAULT_DB_NAME):
	...
	for w in _iterWidgets(formProps):
		...
		cacheKey = "%s:%s" % (enumName, str(enumVersion) if enumVersion is not None else "latest")
		...
		try:
			...
			log.warn("evt=params.expandTemplateForUi widget=%s type=%s err=%s" % (wId, wType, e))
...
def listTemplates(workflowName, status=STATUS_ACTIVE, latestOnly=True, dbName=DEFAULT_DB_NAME):
	"""
...
def getTemplate(workflowName, templateName, templateVersion=None, dbName=DEFAULT_DB_NAME, activeOnly=True):
	"""
...
def createTemplateVersion(workflowName, templateName, formProps, description=None, createdBy=None, activate=True, dbName=DEFAULT_DB_NAME):
	"""
	...
	try:
		...
		db.u(sqlIns, [
			workflowName, templateName, int(nextVersion),
			system.util.jsonEncode(schemaObj),
			status, description, createdBy, long(now_ms())
		], tx=tx)
		...
		db.rollback(tx)
		log.error("evt=params.createTemplateVersion workflow=%s template=%s err=%s" % (workflowName, templateName, e))
		return {"ok": False, "error": str(e)}
	finally:
		db.close(tx)


def setTemplateStatus(workflowName, templateName, templateVersion, status, actor=None, dbName=DEFAULT_DB_NAME):
	"""
	...
	WHERE workflow_name = ? AND template_name = ? AND template_version = ?
	"""
	n = db.u(sql, [statusDb, workflowName, templateName, int(templateVersion)])
	return {"ok": True, "rows": int(n), "actor": actor, "status": statusDb}


def setTemplateActive(workflowName, templateName, templateVersion, isActive, actor=None, dbName=DEFAULT_DB_NAME):
	"""
	...
	return setTemplateStatus(workflowName, templateName, templateVersion, status, actor=actor, dbName=dbName)


def deleteTemplateVersion(workflowName, templateName, templateVersion, actor=None, dbName=DEFAULT_DB_NAME):
	"""
	...
	WHERE workflow_name = ? AND template_name = ? AND template_version = ?
	"""
	n = db.u(sql, [workflowName, templateName, int(templateVersion)])
...
def deleteTemplate(workflowName, templateName, actor=None, dbName=DEFAULT_DB_NAME):
	...
	WHERE workflow_name = ? AND template_name = ?
	"""
	n = db.u(sql, [workflowName, templateName])
	return {"ok": True, "rows": int(n), "actor": actor}


def getTemplateDropdownOptions(workflowName, status=STATUS_ACTIVE, latestOnly=True, dbName=DEFAULT_DB_NAME):
	"""
	...
	Returns:
	    object: Result object returned by this call.
	"""
	items = listTemplates(workflowName, status=status, latestOnly=latestOnly, dbName=dbName)
	out = []
	for t in items:
		label = "%s v%s" % (t.get("templateName"), t.get("templateVersion"))
		val = system.util.jsonEncode(t)#{"templateName": t.get("templateName"), "templateVersion": t.get("templateVersion")})
		out.append({"label": label, "value": val})
	return out


def copyTemplateToWorkflow(sourceWorkflowName, templateName, targetWorkflowName, newTemplateName=None, templateVersion=None, createdBy=None, dbName=DEFAULT_DB_NAME):
	"""
	...
	tpl = getTemplate(sourceWorkflowName, templateName, templateVersion=templateVersion, dbName=dbName, activeOnly=False)
	if not tpl:
	...
	desc = "Copied from %s.%s v%s" % (sourceWorkflowName, templateName, tpl.get("templateVersion"))
...
def createEnumVersion(enumName, enumValues, description=None, createdBy=None, activate=True, dbName=DEFAULT_DB_NAME):
	"""
...
def setEnumStatus(enumName, enumVersion, status, actor=None, dbName=DEFAULT_DB_NAME):
	...
	WHERE enum_name = ? AND enum_version = ?
	"""
	n = db.u(sql, [statusDb, enumName, int(enumVersion)])
	return {"ok": True, "rows": int(n), "actor": actor, "status": statusDb}
...
def deleteEnumVersion(enumName, enumVersion, actor=None, dbName=DEFAULT_DB_NAME):
	...
	WHERE enum_name = ? AND enum_version = ?
	"""
	n = db.u(sql, [enumName, int(enumVersion)])
	return {"ok": True, "rows": int(n), "actor": actor}
...
def resolveInputs(workflowName, templateName=None, overridesData=None, templateVersion=None, actor=None, dbName=DEFAULT_DB_NAME):
	"""
	...
	tpl = getTemplate(workflowName, templateName, templateVersion=templateVersion, dbName=dbName, activeOnly=False)
	if not tpl:
		return {"ok": False, "inputs": None, "errors": ["template_not_found"], "warnings": []}
	...
	for k in overridesData.keys():
		if k not in paramKeys:
			return {"ok": False, "inputs": None, "errors": ["override_key_not_in_template:%s" % k], "warnings": []}
...
def startWithTemplate(workflowName, templateName, overridesData=None, templateVersion=None, actor="admin",
					  queueName="default", partitionKey=None, priority=0,
					  deduplicationId=None, applicationVersion=None, timeoutSeconds=None,
